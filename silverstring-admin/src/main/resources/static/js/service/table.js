...
function attachDataInTable(title, modifyTitle, getApi, addApi, editApi, delApi, fields, addFields, editFields, deleteFields, pageNo, pageSize) {
    if (fields == null) {
    ...
    $.ajax({
        ...
        , success: function (result) {
            ...
            for (var i in result.data.contents) {
                var actions = '<td><button type="button" class="btn btn-sm btn-info" onclick="onEditClick' + '('+  "'" + title + "'" + ',' + "'" + editApi + "'" + ',' + 'rows[' + i + ']' +  ');">' + modifyTitle + '</button> '
            ...
            attachPage(title, modifyTitle, getApi, addApi, editApi, delApi, fields, addFields, editFields, deleteFields, pageNo, pageSize, result.data);
        }
    });
}
...
function onEditClick(title, api, content) {
    ...
    $.each( storageEditFields, function( key, value ) {
        ...
        if (key == 'level') {
            ...
            cell += '<option value="LEVEL1" ' + checkSelectedComboBox(content[key], 'LEVEL1') + '>' + 'LEVEL1' + '</option>';
            cell += '<option value="LEVEL2" ' + checkSelectedComboBox(content[key], 'LEVEL2') + '>' + 'LEVEL2' + '</option>';
            cell += '<option value="LEVEL3" ' + checkSelectedComboBox(content[key], 'LEVEL3') + '>' + 'LEVEL3' + '</option>';
            cell += '</select>';
        } else if (key == 'status') {
            cell += '<select name="' + key + '" class="form-control">';
            cell += '<option value="PENDING" ' + checkSelectedComboBox(content[key], 'PENDING') + '>' + 'PENDING' + '</option>';
            cell += '<option value="CANCEL" ' + checkSelectedComboBox(content[key], 'CANCEL') + ' >' + 'CANCEL' + '</option>';
            cell += '<option value="COMPLETED" ' + checkSelectedComboBox(content[key], 'COMPLETED') + ' >' + 'COMPLETED' + '</option>';
            cell += '<option value="APPROVAL" ' + checkSelectedComboBox(content[key], 'APPROVAL') + ' >' + 'APPROVAL' + '</option>';
            cell += '</select>';
        } else if (key == 'content') {
...
function onAddClick(title, api, content) {
    ...
    $.each( storageAddFields, function( key, value ) {
        ...
        if (key == 'level') {
            cell += '<select name="' + key + '" class="form-control">';
            cell += '<option value="LEVEL1" selected>' + 'LEVEL1' + '</option>';
            cell += '<option value="LEVEL2" >' + 'LEVEL2' + '</option>';
            cell += '<option value="LEVEL3" >' + 'LEVEL3' + '</option>';
            cell += '</select>';
        } else if (key == 'status') {
            cell += '<select name="' + key + '" class="form-control">';
            cell += '<option value="PENDING" selected>' + 'PENDING' + '</option>';
            cell += '<option value="CANCEL" >' + 'CANCEL' + '</option>';
            cell += '<option value="COMPLETED" >' + 'COMPLETED' + '</option>';
            cell += '<option value="APPROVAL" >' + 'APPROVAL' + '</option>';
            cell += '</select>';
        } else if (key == 'content') {
...
function attachHeaders(fields) {
    var headersContent = '<th class="sorting" tabindex="0" aria-controls="datatable-keytable" rowspan="1" colspan="1" aria-label="Office: activate to sort column ascending" style="color:gray;width: 140px;">' + "액션" + '</th>';;
...
function attachPage(title, modifyTitle, getApi, addApi, editApi, delApi, fields, addFields, editFields, deleteFields, pageNo, pageSize, data) {
    //page
